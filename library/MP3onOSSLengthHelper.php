...
class MP3onOSSLengthHelper
{
     ...
     * @var int
     */
    protected $ossObjectContentPointer;
    /**
     * @var int
     */
    protected $ossObjectLength;

    protected $ossObjectBuffer;

    public function __construct($oss, $bucket, $object)
    {
        $this->ossBucket = $bucket;
        $this->ossObject = $object;
        $this->ossObjectContentPointer = 0;

        $this->oss = $oss;

        $meta = $this->oss->getObjectMeta($this->ossBucket, $this->ossObject);
        $this->ossObjectLength = $meta['content-length'];

        $this->ossObjectBuffer = [];
    }
    ...
    {
        $duration = 0;
        $block = $this->ossObjectRead(100);
        $offset = $this->skipID3v2Tag($block);
        $this->ossObjectSeek($offset, SEEK_SET);
        while (
            $this->ossObjectContentPointer < $this->ossObjectLength
        ) {
            $block = $this->ossObjectRead(10);
            if (strlen($block) < 10) {
                break;
            } //looking for 1111 1111 111 (frame synchronization bits)
            else if ($block[0] == "\xff" && (ord($block[1]) & 0xe0)) {
                $info = self::parseFrameHeader(substr($block, 0, 4));
                $this->ossObjectSeek($info['Framesize'] - 10, SEEK_CUR);
                $duration += ($info['Samples'] / $info['Sampling Rate']);
            } else if (substr($block, 0, 3) == 'TAG') {
                $this->ossObjectSeek(128 - 10, SEEK_CUR);
            } else {
                $this->ossObjectSeek(-9, SEEK_CUR);
            }
    ...
    {
        $kbps = ($bitrate * 1000) / 8;
        $datasize = $this->ossObjectLength - $offset;
    ...
    protected function ossObjectRead($length)
    {
        $bufferSize = 10240;

        if ($this->ossObjectContentPointer + $length > $this->ossObjectLength) {
            $length = $this->ossObjectLength - $this->ossObjectContentPointer + 1;
        }
         ...
         * [6 7 8]
         * ...
         * frame (2,7) length=6 would be
         * (1) 2/3=0, buffer[0][2%3] to end
         * (2) buffer[1][max]<7, buffer[1] full
         * (3) buffer[2][max]>7, end at buffer[2][7%3]
         */

        $bufferMinIndex = (int)(($this->ossObjectContentPointer - $this->ossObjectContentPointer % $bufferSize) / $bufferSize);
        $bufferMaxIndex = (int)((($this->ossObjectContentPointer + $length - 1) - ($this->ossObjectContentPointer + $length - 1) % $bufferSize) / $bufferSize);

        echo __METHOD__ . '@' . __LINE__ . ' current ' . $this->ossObjectContentPointer . ' length ' . $length . " buffer ($bufferMinIndex,$bufferMaxIndex)" . PHP_EOL;

        $buffer = "";
        for ($bufferIndex = $bufferMinIndex; $bufferIndex <= $bufferMaxIndex; $bufferIndex++) {
            if (!isset($this->ossObjectBuffer[$bufferIndex])) {
                echo "Try to load buffer range " . ($bufferIndex * $bufferSize) . '-' . ($bufferIndex * $bufferSize + $bufferSize - 1) . PHP_EOL;
                $this->ossObjectBuffer[$bufferIndex] = $this->oss->getObject($this->ossBucket, $this->ossObject, [
                    OssClient::OSS_RANGE => ($bufferIndex * $bufferSize) . '-' . ($bufferIndex * $bufferSize + $bufferSize - 1)
                ]);
            }

            $startIndexInBuffer = 0;
            $endIndexInBuffer = $bufferSize - 1;
            if ($bufferIndex == $bufferMinIndex) {
                $startIndexInBuffer = $this->ossObjectContentPointer % $bufferSize;
            }
            if ($bufferIndex == $bufferMaxIndex) {
                $endIndexInBuffer = ($this->ossObjectContentPointer + $length - 1) % $bufferSize;
            }

            $buffer .= substr($this->ossObjectBuffer[$bufferIndex], $startIndexInBuffer, $endIndexInBuffer - $startIndexInBuffer + 1);
        }

//        foreach ($this->ossObjectBuffer as $key => $value){
//            echo "BUFFER[$key]: length=".strlen($value).PHP_EOL;
//        }

        //would timeout around 31365120 it would not happen every time but a possibility

//        $content=$this->oss->getObject($this->ossBucket,$this->ossObject,[
//            OssClient::OSS_RANGE=>$this->ossObjectContentPointer.'-'.($this->ossObjectContentPointer+$length-1)
//        ]);
        $this->ossObjectContentPointer += $length;
        return $buffer;
    }
     ...
     * @param int $whence
     */
    protected function ossObjectSeek($offset, $whence = SEEK_SET)
    {
        switch ($whence) {
            case SEEK_CUR:
                //- Set position to current location plus offset.
                $this->ossObjectContentPointer += $offset;
                break;
            case SEEK_END:
                // - Set position to end-of-file plus offset. // offset should be negative
                $this->ossObjectContentPointer = $this->ossObjectLength + $offset;
                break;
            case SEEK_SET:
            default:
